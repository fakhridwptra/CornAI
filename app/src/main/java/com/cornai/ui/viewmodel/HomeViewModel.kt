...
// File: java\com\cornai\ui\viewmodel\HomeViewModel.kt
...
class HomeViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun refreshWeather() {
        val conditions = listOf(
            WeatherData("Cerah", 32, "Ideal untuk menyemrot pestisida", "🌤️", "Siang"),
            WeatherData("Cerah Berawan", 30, "Cocok untuk pemupukan nitrogen", "⛅", "Siang"),
            WeatherData("Berawan", 28, "Bagus untuk memeriksa penyakit daun", "☁️", "Siang"),
            WeatherData("Mendung", 27, "Suhu ideal, angin mungkin bertiup kencang", "🌥️", "Sore"),
            WeatherData("Hujan Ringan", 24, "Tunda penyemprotan, cari perlindungan", "🌧️", "Sore"),
        ...
        val filtered = conditions.filter { it.condition != current.condition }
        val next = if (filtered.isNotEmpty()) filtered.random() else conditions.random()
        val randomOffset = (-1..1).random()
        _weatherState.value = next.copy(temperature = next.temperature + randomOffset)
    }
